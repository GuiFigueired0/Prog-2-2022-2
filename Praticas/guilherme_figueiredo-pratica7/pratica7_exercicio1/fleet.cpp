...
std::vector<Aeroplane*> Fleet::instantiate_fleet(int number_of_planes) {
	...
	//Aeroplane *hercules; // n faz sentido ter um aviao que não vai ser inicializado
    ...
    //fleet.push_back(hercules); // hercules nao foi instanciado 
	//                              então não tem porque passar ele
...
void Fleet::print_fleet_data(std::vector<Aeroplane*> fleet) {
    ...
    for (int i=0; i<fleet.size();i++) {
		...
		if (Fighter_Aircraft* aircraft = dynamic_cast<Fighter_Aircraft*>(fleet[i])){
			...
			//aircraft->shoot(); // nao faz sentido atirar quando vc so quer ver as informacoes 
		    //std:: cout << "Number of missiles: " << aircraft->get_number_of_missiles() << std::endl;
		}

		if (Jet_Plane* aircraft = dynamic_cast<Jet_Plane*>(fleet[i])){
			std:: cout << "Company: " << aircraft->get_company() << std::endl;
		}
